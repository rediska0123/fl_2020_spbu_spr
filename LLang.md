...
* Допускается писать любое количество пробелов, знаков табуляции и/или переносов строки в любых местах, кроме позиций внутри ключевых слов `Def` и `Returns`. Например, допускается вообще не использовать пробелы: `Deffunc(x)Returns(x);`
