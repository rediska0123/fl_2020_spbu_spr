...
Допускается писать скобки для правильного расставления приоритетов; пробелы, символы табуляции и/или переносы строки в любом месте выражения (но не внутри числа, имени переменной или многосимвольного бинарного оператора).
...
* Допускается писать любое количество пробелов, знаков табуляции и/или переносов строки в любых местах, кроме позиций внутри ключевых слов (`Assign`, `Read`, `Write`, `If`, `While`). Например, допускается вообще не использовать пробелы: `Readx`


## Функции
Функцию можно определить, используя следующий синтаксис.

Пусть `function_name, arg1, ..., argn` – любые корректные имена переменных (n – натуральное число, либо 0), `body` – команда, образованная с помощью правила **List of commands** выше, `ret` – выражение.

Тогда любая строка, образованная каким-либо из следующих правил, является корректным определением функции в LLang:

Операция                | Ситнаксис___________________________________________ | Определение        
------------------------|----------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------
**Function Definition** | `Def function_name(arg1, ..., argn) body Returns (ret);` | определение функции
**Function Definition** | `Def function_name(arg1, ..., argn) Returns (ret);` | определение функции без тела

* Функция может быть названа так-же, как и какая-то переменная.
* Все переменные, используемые внутри функции в `body` или `ret`, но не определенные заранее, по умлочанию изначально имеют значение `0`.
* Внутри `body` все правила синтаксиса аналогичные правилам в **List of commands**: в том числе разрешается ставить пробелы где угодно, кроме мест внутри ключевых слов.
* Кроме `body` в определении функции допускается писать любое количество пробелов, знаков табуляции и/или переносов строки в любых местах, кроме позиций внутри ключевых слов `Def` и `Returns`. Например, допускается вообще не использовать пробелы: `Deffunc(x)Returns(x);`

Примеры функций:
- `Def func(x, y) { Read n; } Returns (n+x+y);`
- `Def mult(x, y) Returns (x*y);`
...
Тогда конструкция `f1 f2 ... fn main` является корректной программой на LLang.

* Допускается писать любое количество пробелов, знаков табуляции и/или переносов строки в любых местах, кроме позиций внутри ключевых слов (`Assign`, `Read`, `Write`, `If`, `While`, `Def`, `Returns`) и позиций внутри цифр, имен переменных и операторов.
...
```
```
Def f(x, y) Returns (x+y); Def g(x) { Assign r (x*x); } Returns (r); { Write (f(g(3), g(4))); }
```
```
Def f() { } Returns (0); { }
```
...
```
```
Def f() Returns (0); Def f(x) Returns (x); { }
```
```
Def f() Returns (0); { Write (f(2)); }
```
```
{ Write (f(2)); }
```
