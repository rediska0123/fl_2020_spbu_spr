...
Переменные, которые используются без предварительного объявления с помощью **Assign** (например, в программе `{ Write (x); }`), будут заведены в месте их использования с начальным значением 0 (например, в последней программе будет выведено число `0`).
