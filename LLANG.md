...
Пусть a, b – корректные выражения в языке LLang.

Любые конструкции, образованные по правилам ниже, являются корректными выражениями в языке LLang.

Операция | Ситнаксис | Определение                                                                          | Ассоциативность   | Приоритет
---------|-----------|--------------------------------------------------------------------------------------|-------------------|----------
число    | десятичная запись: например `12` | любое натуральное число или 0                                          |                   | 0
переменная | имя переменной: например `x` | любое корректное имя                                         |                   | 0
 `-`     | `-a`      | значение, противоположное результату вычисления выражения `a`                        |                   | 1
 `\|\|`  | `a\|\|b`  | 1, если хотя бы один из результатов вычисления `a` и `b` не равен нулю, 0 иначе      | Правоассоциативна | 2
 `&&`    | `a&&b`    | 1, если оба результата вычисления `a` и `b` не равны нулю, 0 иначе                   | Правоассоциативна | 3
 ...
 `^`     | `a^b`     | возведение результата вычисления выражения `a` в степень результата `b`              | Правоассоциативна | 7
 
Первыми считаются операции с наибольшим приоритетом.
